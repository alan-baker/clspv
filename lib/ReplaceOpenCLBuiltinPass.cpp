...
bool ReplaceOpenCLBuiltinPass::replaceRound(Function &F) {
  return replaceCallsWithValue(F, [&F](CallInst *Call) {
    const auto x = Call->getArgOperand(0);
    const auto ty = Call->getType()->getScalarType();
    const double c_halfway = 0.5;
    //const double c_one = 1.0;
    Constant *halfway = ConstantFP::get(ty, c_halfway);
    //Constant *one = ConstantFP::get(ty, c_one);
    if (auto vec_ty = dyn_cast<VectorType>(Call->getType())) {
      halfway = ConstantVector::getSplat(vec_ty->getElementCount(), halfway);
      //one = ConstantVector::getSplat(vec_ty->getElementCount(), one);
    }
    ...
    auto copysign = Intrinsic::getDeclaration(
        F.getParent(), Intrinsic::copysign, Call->getType());
    //auto trunc = Intrinsic::getDeclaration(F.getParent(), Intrinsic::trunc, Call->getType());

    IRBuilder<> builder(Call);
    //auto copy_halfway = builder.CreateCall(F.getFunctionType(), copysign, {halfway, x});
    //auto sub = builder.CreateFSub(x, copy_halfway);
    //auto truncated = builder.CreateCall(F.getFunctionType(), trunc, {sub});
    //auto copy_one = builder.CreateCall(F.getFunctionType(), copysign, {one, x});
    //return builder.CreateFAdd(truncated, copy_one);
...
bool ReplaceOpenCLBuiltinPass::replaceTrigPi(Function &F, Builtins::BuiltinType type) {
  return replaceCallsWithValue(F, [&F, type](CallInst *Call) -> Value * {
    ...
    switch (type) {
    case Builtins::kSinpi: {
      auto func = Intrinsic::getDeclaration(F.getParent(), Intrinsic::sin, x->getType());
      return builder.CreateCall(func->getFunctionType(), func, {mul});
    }
    case Builtins::kCospi: {
      auto func = Intrinsic::getDeclaration(F.getParent(), Intrinsic::cos, x->getType());
      return builder.CreateCall(func->getFunctionType(), func, {mul});
    }
    case Builtins::kTanpi: {
      auto sin = Intrinsic::getDeclaration(F.getParent(), Intrinsic::sin, x->getType());
      auto sin_call = builder.CreateCall(sin->getFunctionType(), sin, {mul});
      auto cos = Intrinsic::getDeclaration(F.getParent(), Intrinsic::cos, x->getType());
